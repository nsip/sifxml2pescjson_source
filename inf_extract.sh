...
rm -f objs.txt
xsltproc included_objects.xslt $1/06_DataModel/Custom/DataModel-Infrastructure.xml | perl -ne 'next unless $_ =~ /\S/; next if $_ =~ /<\?/; s/^\s+//; s/\s+$//;  @a = split("/", $_); print $a[-1] . "\n"' > objs.txt
...
grep "schemaVersion" $1/SIF.Config_DataModel_IN.xml | perl -pe 's/\s*<schemaVersion>/VERSION: /; s#</schemaVersion>##; $$_;' > scripts/out.txt
